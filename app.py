# app.py — Destajo con Planeación PRIV + Captura + Tablero + Admin (con fix ESPERA/PLAN_CODE)
...
if qp.get("api", [None])[0] == "ingest":
    ...
    allowed = True if token else False  # cambia esto si usas secrets
    if not allowed:
        st.write({"ok": False, "error": "TOKEN_INVALIDO"}); st.stop()
    try:
        payload = json.loads(base64.urlsafe_b64decode(qp.get("data",[None])[0] + "==="))
